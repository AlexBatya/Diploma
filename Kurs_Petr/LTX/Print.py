...
LatexTable(VV,r'texi/V','V',r'Результаты расчётов $V(M,H)$ м/с',1)
...
#     plt.plot(M_interp,P_potr[i], marker = 'x')
#     plt.plot(M_interp,P_rasp[i],marker = '^')
...
# plt.plot(M_min,H_interp,'-',M_max,H_interp,'-.',MCy_dop,H_interp,'--', M_pred, H_interp)
# plt.grid()
# plt.xlabel('Число Маха')
...
plt.plot(H_interp,qh_min1,H_interp,qkm_min,'--')
plt.grid()
plt.xlabel('Часовой и километровый расходы топлива, кг/ч(кг/км)')
plt.legend((r'$q_{ч_{min}} \cdot 10^{-3}$, кг/ч',r'$q_{км_{min}}$, кг/км'))
plt.ylabel('Высота, м')
